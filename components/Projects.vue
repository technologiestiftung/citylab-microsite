<template>
  <section id="team" class="section projects is-medium">
    <div class="container">
      <h2 class="title">
        {{ this.content[this.lang].projects.title }}
      </h2>
      <h2
        class="subtitle"
...
<script>
  ...
  export default {
    ...
    created() {
      axios
        .get(
          `https://spreadsheets.google.com/feeds/list/1rTyfInS6NjTifbru61mWEqICyv9uuMVSSk7NZTABLQc/${this.sheetId}/public/values?alt=json`
        )
        .then((res) => {
        ...
        .catch((err) => {
          console.error(err, 'Projects.vue');
        });
    },
  };
</script>
