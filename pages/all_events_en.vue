<template>
  <div>
    ...
    <section class="section is-medium schedule">
      ...
      >
        <h1 class="title">
          {{ content[this.lang]['events']['upcomingEventsTitle'] }}
        </h1>
        ...
        <div class="schedule-wrapper">
          ...
          >
            <article class="dates-item" style="width: 100%;">
              ...
              <div
                style="margin-right: 0px; width: 240px;"
                class="date-wrapper"
              >
      ...
      <div v-if="dataPast" class="container">
        <h1 class="title">
          {{ content[this.lang]['events']['pastEventsTitle'] }}
        </h1>

        <div class="schedule-wrapper">
          <a v-for="entry in dataPast" class="event-tile" :href="entry['link']">
            <article class="dates-item" style="width: 100%;">
              <div class="date-wrapper">
                <span class="date-month"> {{ entry.month }} </span>
                <span class="date-day"> {{ entry.day }} </span>
  ...
  export default {
    ...
    mounted() {
      axios
        ...
        .catch((err) => {
          console.error(err, 'all_events_en.vue');
        });
    },
    methods: {
      filterData(d, isUpcoming) {
        return d.filter(
          (entry) =>
            entry.visible !== 'FALSE' &&
            (isUpcoming
              ? this.dateIsUpcoming(entry.date)
              : !this.dateIsUpcoming(entry.date)) &&
