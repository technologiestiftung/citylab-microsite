<template>
  <div>
    <Navigation :lang="lang" :content="content" :direct="direct" :anchor-tags="true" />
    <HeroLight
      :image="heroImageUrl"
      :title="title"
      :subTitle="subtitle"
...
{{ headlineIntro }}
          ...
          <p v-if="summaryAvailable.introContent > 0" v-html="contentIntro" class="event-intro"/>
        </div>
...
{{ headlineBlockOne }}
          ...
          <p v-if="summaryAvailable.blockOneContent > 0" v-html="contentBlockOne" class="event-intro"/>
        </div>
...
{{ headlineBlockTwo }}
          ...
          <p v-if="summaryAvailable.blockTwoContent > 0" v-html="contentBlockTwo" class="event-intro"/>
        </div>
...
{{ headlineBlockThree }}
          ...
          <p v-if="summaryAvailable.blockThreeContent > 0" v-html="contentBlockThree" class="event-intro"/>
        </div>
...
{{ headlineBlockFour }}
          ...
          <p v-if="summaryAvailable.blockFourContent > 0" v-html="contentBlockFour" class="event-intro"/>
        </div>

        <div class="logo-wrapper">
          <a v-if="link.length > 0" :href="link" target="_blank" class="button is-color-secondary is-normal">{{ lang == 'en' ? 'To the project' : 'Zum Projekt' }}</a>
        </div>
</div>
    </section>
      ...
      contentBlockFour() {
        ...
        beforeCreate() {
            axios.get(`https://spreadsheets.google.com/feeds/list/1rTyfInS6NjTifbru61mWEqICyv9uuMVSSk7NZTABLQc/1/public/values?alt=json`)
              ...
              console.error(err, "_project.vue")
            });
